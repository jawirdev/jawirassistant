...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // (BARU) Halaman 8 (Spotify Download)
    ...
    // ==================================
    function toggleMenu() { /* ... (Tetap sama) ... */ 
    ...
    function showPage(pageId) { /* ... (Tetap sama) ... */
        pages.forEach(page => { if (page.id === pageId) { setTimeout(() => { page.classList.add('active'); if (pageId === 'page-analytics') loadAnalyticsCharts(); }, 100); } else { page.classList.remove('active'); } });
    }
    ...
    // ==================================
    function loadDeviceInfo() { /* ... (Tetap sama) ... */
        function updateTime() { const now = new Date(); infoTime.textContent = now.toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' }); } updateTime(); setInterval(updateTime, 1000);
        infoDevice.textContent = navigator.userAgentData?.platform || navigator.platform || 'Tidak diketahui';
        if ('getBattery' in navigator) { navigator.getBattery().then(battery => { function updateBatteryStatus() { const level = (battery.level * 100).toFixed(0); infoBattery.textContent = `${level}% ${battery.charging ? '(Mengisi daya)' : '(Tidak mengisi)'}`; } updateBatteryStatus(); battery.addEventListener('levelchange', updateBatteryStatus); battery.addEventListener('chargingchange', updateBatteryStatus); }); } else { infoBattery.textContent = 'Tidak didukung'; }
        const connection = navigator.connection || navigator.mozConnection || navigator.webkitConnection; infoNetwork.textContent = connection ? (connection.effectiveType ? `${connection.effectiveType.toUpperCase()}` : 'Tidak diketahui') : 'Tidak didukung';
        fetch('https://ipapi.co/json/').then(response => response.json()).then(data => { infoIp.textContent = data.ip || 'Gagal memuat'; infoLocation.textContent = `${data.city}, ${data.region}, ${data.country_name}`; }).catch(() => { infoIp.textContent = 'Gagal memuat'; infoLocation.textContent = 'Gagal memuat'; });
    }
    loadDeviceInfo();
    ...
    function displayError(message, containerId) { /* ... (Tetap sama) ... */
        const container = document.getElementById(containerId); if (container) { container.innerHTML = `<div class="error-message">${message}</div>`; }
    }
    ...
    function displayTikTokResults(data, containerId) { /* ... (Tetap sama) ... */
        const container = document.getElementById(containerId); const metadata = data.metadata; const downloads = data.download; let title = metadata.title || metadata.description || 'Video TikTok'; if (title.length > 100) title = title.substring(0, 100) + '...';
        let downloadLinksHTML = '';
        if (downloads.video && downloads.video.length > 0) { downloads.video.forEach((videoUrl, index) => { let qualityLabel = `Video ${index + 1}`; if (videoUrl.includes('original')) qualityLabel = 'Video Original (HD)'; downloadLinksHTML += `<a href="${videoUrl}" target="_blank" download>Unduh ${qualityLabel}</a>`; }); } else { downloadLinksHTML = '<p>Tidak ada link video yang ditemukan.</p>'; }
    ...
    function displayYouTubePlayResults(data, containerId) { /* ... (Tetap sama) ... */
        const container = document.getElementById(containerId); const metadata = data.metadata;
    ...
    function displayTranscriptResults(data, containerId) {
        ...
        const textId = "transcript-text-1"; // ID unik untuk teks
        container.innerHTML = `
    ...
    function displaySummaryResults(data, containerId) {
        const container = document.getElementById(containerId);
        const textId = "summary-text-1"; // ID unik untuk teks
    ...
    // ==================================
    // (BARU) FUNGSI HALAMAN 7: SPOTIFY SEARCH
    // ==================================
    ...
    // ==================================
    // (BARU) FUNGSI HALAMAN 8: SPOTIFY DOWNLOAD
    // ==================================
    ...
    // ==================================
    // FUNGSI HALAMAN 9: ANALITIK (Tetap)
    // ==================================
    function loadAnalyticsCharts(forceReload = false) { /* ... (Tetap sama) ... */
        ...
        if (!featuresChartInstance) { const ctxFeatures = document.getElementById('features-chart').getContext('2d'); featuresChartInstance = new Chart(ctxFeatures, { type: 'doughnut', data: { labels: ['TikTok', 'YT Play', 'Spotify', 'Lainnya'], datasets: [{ label: 'Penggunaan Fitur', data: [450, 110, 280, 50], backgroundColor: [primaryColor, '#ff3b30', '#1DB954', '#ff9500'], borderColor: 'transparent', }] }, options: { responsive: true, plugins: { legend: { position: 'bottom', labels: { color: textColor } } } } }); }
    }
